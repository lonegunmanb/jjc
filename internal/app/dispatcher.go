...
func assembleTerminateNotice(rawBody, slimBody []byte, reason string) string {
	...
	b.WriteString("3. Delete your assigned work_dir. Its absolute path is in the CARD CONTEXT block ")
	b.WriteString("at the top of your system prompt (look for the `work_dir` field). Use whatever ")
	b.WriteString("shell command is appropriate for the OS the path indicates (`Remove-Item -Recurse ")
	b.WriteString("-Force ...` on Windows, `rm -rf ...` on Unix).\n")
